...
initial_values = [objective_function(np.sin(initial_point)) for initial_point in initial_points]
...
for i in range(num_iterations):
    ...
    suggested_waves = [optimizer.ask() for _ in range(4)]
    ...
    distances = [objective_function(np.sin(x_values + np.array(wave))) for wave in suggested_waves]

    # Visualize the suggested waves every 10 iterations
    if i % 50 == 0:
        visualize_wave(x_values, [np.sin(x_values + np.array(wave)) for wave in suggested_waves], target_wave)

    # Provide feedback to the optimizer about all the waves
